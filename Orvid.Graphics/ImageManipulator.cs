...
namespace Orvid.Graphics
{
    public static class ImageManipulator
	{
		...
		#region Greyscale to Transparent
		public static Image GreyscaleToTransparent(Image src)
		{
			Image o = new Image(src.Size);
			for (uint y = 0; y < o.Height; y++)
			{
				for (uint x = 0; x < o.Width; x++)
				{
					Pixel p2 = src.GetPixel(x, y);
					Pixel final = new Pixel(0x00, 0x00, 0x00, (byte)(255 - p2.R));
					o.Data[(y * o.Width) + x] = final;
				}
			}
			return o;
		}
		...
		#region Color To Transparent
		//public static Image ColorToTransparent(Image src, Pixel color)
		//{
		//    Image o = new Image(src.Size);
		//    PixelF p = color / 255;
		//    for (uint y = 0; y < o.Height; y++)
		...
		//            Pixel p2 = src.GetPixel(x, y);
		//            PixelF al = p2 / p;
		//            int alpha = (int)((al.R + al.G + al.B) / 3);
		//            //Console.WriteLine("Alpha: {0}", alpha);
		//            Pixel final = new Pixel((byte)Math.Abs(255 - (p2.R - color.R)), (byte)Math.Abs(255 - (p2.G - color.G)), (byte)Math.Abs(255 - (p2.B - color.B)), (byte)(255 - alpha));
		//            o.SetPixel(x, y, final);
		//        }
		//    }
		//    return o;
		//}
		...
		public static Image FlipV(Image i)
		{
			Image o = new Image(i.Size);

			uint y2 = 0;
			for (int y = o.Height - 1; y >= 0; y--)
			{
				for (uint x = 0; x < o.Width; x++)
				{
					o.SetPixel(x, y2, i.GetPixel(x, (uint)y));
				}
				y2++;
			}
        ...
        public static Image Resize(Image i, Vec2 outSize, ScalingAlgorithm method)
        {
            Image o;
        ...
        private static Image ResizeNearestNeighbor(Image i, Vec2 outSize)
        {
			...
			double x_ratio = (double)(i.Width)  / (double)(o.Width);
			double y_ratio = (double)(i.Height) / (double)(o.Height);
            uint px, py;
            for (uint y = 0; y < o.Height; y++)
            {
                for (uint x = 0; x < o.Width; x++)
