...
namespace Orvid.TrueType
{
	public static class SubPixelResize
	{
		...
		public static Image SubPixelAntiAliasX2(Image img)
		{
			...
			for (uint y = 0; y < img.Height - 1; y += ratio, fY++)
			{
				fX = 0;
				for (uint x = 0; x < img.Width - 1; x += ratio, fX++)
				{
		...
		{
#warning Need to do an unsafe version of this, this uses a large number of array lookups.
			const int ratio = 4;
			...
			uint r, g, b, a;
			//fixed (Pixel* sarrBase = img.Data)
			//{
			//    fixed (Pixel* darrBase = fImg.Data)
			//    {
			//        Pixel* 
			//    }
			//}
